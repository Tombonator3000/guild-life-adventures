...
export function createNpcFavorActions(set:SetFn,get:GetFn) {
  return { performNpcFavor(playerId:string,favorId:string):ActionResult {
    ...
    set(s=>({players:s.players.map(p=>p.id===playerId ? {...p,...cityActivityEffects(p,favor),infamy:Math.min(100,(p.infamy??0)+(favor.infamy??0)),claimedNpcFavors:[...(p.claimedNpcFavors??[]),favor.id],gameStats:{...p.gameStats,totalGoldEarned:(p.gameStats.totalGoldEarned??0)+favor.gold}}:p)}));
