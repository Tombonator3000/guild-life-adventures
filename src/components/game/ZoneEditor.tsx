...
import { ZoneEditorHomePanel } from './ZoneEditorHomePanel';
...
export function ZoneEditor({ onClose, onSave, onReset, initialCenterPanel, initialZones, initialPaths, initialLayout, initialAnimationLayers, initialMobileOverrides, initialHomeItemPositions }: ZoneEditorProps) {
  const state = useZoneEditorState({ onClose, onSave, onReset, initialCenterPanel, initialZones, initialPaths, initialLayout, initialAnimationLayers, initialMobileOverrides, initialHomeItemPositions });
  ...
  return (
    <div className="fixed inset-0 bg-black/90 z-50 flex flex-col">
      ...
      <div className="flex-1 flex overflow-hidden">
        {state.editorMode === 'home' ? (
          <ZoneEditorHomePanel
            containerRef={state.containerRef}
            handleMouseMove={state.handleMouseMove}
            homeItemPositions={state.homeItemPositions}
            homeRoomType={state.homeRoomType}
            setHomeRoomType={state.setHomeRoomType}
            selectedHomeItem={state.selectedHomeItem}
            setSelectedHomeItem={state.setSelectedHomeItem}
            handleHomeItemMouseDown={state.handleHomeItemMouseDown}
            setHomeItemPositions={state.setHomeItemPositions}
          />
        ) : (
          <>
            <ZoneEditorBoard
          ...
          : state.editorMode === 'paths'
            ...
            : state.editorMode === 'animations'
              ...
              : state.editorMode === 'mobile'
                ...
                : state.editorMode === 'home'
                  ? 'Home layout: Drag items to reposition them | Click to select | Apply & Save to persist positions to the live game'
