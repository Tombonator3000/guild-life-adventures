...
import type { Player, GoalSettings, LocationId } from '@/types/game.types';
...
function DeveloperTab({
  ...
  const debugBtn = 'w-full flex items-center justify-center gap-1 p-1.5 bg-amber-100/30 hover:bg-amber-200 rounded border border-amber-300/50 text-amber-700 font-display text-[10px] transition-colors';
  const smallBtn = 'flex-1 p-1 bg-amber-100/30 hover:bg-amber-200 rounded border border-amber-300/50 text-amber-700 font-display text-[9px] transition-colors text-center';
  ...
  return (
    <div className="space-y-2">
      ...
      <OptionSection title="Zone Editor">
        <button onClick={onToggleZoneEditor} className={debugBtn}>
          Open Zone Editor
        </button>
        ...
        <div className="grid grid-cols-3 gap-1">
          {([
            ...
            <button
              ...
              onClick={() => {
                if (type === 'clear') {
                  store.setDebugWeather('clear');
                } else {
                  store.setDebugWeather(type);
                }
              }}
              className={`${smallBtn} ${store.weather?.type === type ? 'bg-amber-300 border-amber-600 text-amber-900' : ''}`}
            >
              {label}
            </button>
          ))}
      ...
      <OptionSection title="Victory & Game">
        <div className="space-y-1">
          <button
            onClick={() => {
              if (playerId) {
                // Set all goals to met instantly
          ...
          <button
            onClick={() => {
              if (playerId) store.checkVictory(playerId);
            }}
            className={debugBtn}
          >
            Check Victory
          ...
          <button
            onClick={() => {
              if (playerId) store.startTurn(playerId);
            }}
            className={debugBtn}
          >
            Force Start Turn
        ...
        <div className="grid grid-cols-2 gap-1">
          {([
            ...
            <button
              ...
              className={`${smallBtn} ${store.activeFestival === id ? 'bg-amber-300 border-amber-600' : ''}`}
            >
              {label}
            </button>
          ))}
      ...
      <OptionSection title="Resources (Current Player)">
        {playerId && (
          <div className="space-y-1">
            <div className="grid grid-cols-3 gap-1">
              <button onClick={() => store.modifyGold(playerId, 500)} className={smallBtn}>+500g</button>
              <button onClick={() => store.modifyGold(playerId, -500)} className={smallBtn}>-500g</button>
              <button onClick={() => store.modifyGold(playerId, 5000)} className={smallBtn}>+5000g</button>
            </div>
            <div className="grid grid-cols-3 gap-1">
              <button onClick={() => store.modifyHealth(playerId, 50)} className={smallBtn}>+50 HP</button>
              <button onClick={() => store.modifyHealth(playerId, -50)} className={smallBtn}>-50 HP</button>
              <button onClick={() => store.modifyHealth(playerId, 100)} className={smallBtn}>Full HP</button>
            </div>
            <div className="grid grid-cols-3 gap-1">
              <button onClick={() => store.modifyHappiness(playerId, 50)} className={smallBtn}>+50 Hap</button>
              <button onClick={() => store.modifyFood(playerId, 50)} className={smallBtn}>+50 Food</button>
              <button onClick={() => store.modifyClothing(playerId, 50)} className={smallBtn}>+50 Cloth</button>
            </div>
            <div className="grid grid-cols-2 gap-1">
              <button onClick={() => store.spendTime(playerId, -20)} className={smallBtn}>+20 Hours</button>
              <button onClick={() => store.modifyRelaxation(playerId, 20)} className={smallBtn}>+20 Relax</button>
            </div>
          </div>
        )}
      </OptionSection>
      ...
      <OptionSection title="Events">
        <div className="space-y-1">
          ...
          <button onClick={() => store.dismissEvent()} className={debugBtn}>
            Dismiss Event
          </button>
        </div>
      </OptionSection>
      ...
      <OptionSection title="Player State">
        {playerId && (
          <div className="space-y-1">
            <button
              onClick={() => store.cureSickness(playerId)}
              className={debugBtn}
            >
              Cure Sickness
            </button>
            <button
              onClick={() => store.modifyMaxHealth(playerId, 20)}
              className={debugBtn}
            >
              +20 Max Health
            </button>
            <button
              onClick={() => store.buyGuildPass(playerId)}
              className={debugBtn}
            >
              Buy Guild Pass
            </button>
            <button
              onClick={() => store.promoteGuildRank(playerId)}
              className={debugBtn}
            >
              Promote Guild Rank
            </button>
          ...
          <div className="grid grid-cols-3 gap-1">
            {LOCATIONS_LIST.map(loc => (
              <button
                ...
                className={`${smallBtn} ${currentPlayer?.currentLocation === loc.id ? 'bg-amber-300 border-amber-600' : ''}`}
                title={loc.name}
              >
                {loc.short}
              </button>
...
// Short location names for the teleport grid
