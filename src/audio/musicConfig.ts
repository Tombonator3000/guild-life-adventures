// Music configuration — maps game contexts to verified MP3 tracks in /public/music/.
import type { LocationId } from '@/types/game.types';

export interface MusicTrack {
  id: string;
  file: string;
  /** Optional additional files. Only list variants that are packaged and audited. */
  variants?: string[];
  label: string;
}

/** All currently packaged music tracks passed the measurable FFmpeg audit. */
export const MUSIC_TRACKS: Record<string, MusicTrack> = {
  'main-theme':       { id: 'main-theme',       file: '01MainTheme.mp3',              label: 'Main Theme' },
  'on-the-street':    { id: 'on-the-street',    file: '02OnTheStreet.mp3',            label: 'On the Street' },
  'guild-hall':       { id: 'guild-hall',       file: '03guildhall.mp3',              label: 'Guild Hall' },
  'bank':             { id: 'bank',             file: '06Bank.mp3',                   label: 'The Bank' },
  'the-slums':        { id: 'the-slums',        file: '09TheSlums.mp3',               label: 'The Slums' },
  'noble-heights':    { id: 'noble-heights',    file: '10Noble-Heights.mp3',          label: 'Noble Heights' },
  'enchanter':        { id: 'enchanter',        file: '11EnchantersWorkshop.mp3',     label: "Enchanter's Workshop" },
  'rusty-tankard':    { id: 'rusty-tankard',    file: '13rustytankard.mp3',           label: 'The Rusty Tankard' },
  'weekend':          { id: 'weekend',          file: '18OhWhatAWeekend.mp3',         label: 'Oh What a Weekend' },
  'winner':           { id: 'winner',           file: '19Winner.mp3',                 label: 'Victory' },
  'cave':             { id: 'cave',             file: '20Cave.mp3',                   label: 'The Cave' },
  'dragons-lair':     { id: 'dragons-lair',     file: 'Dragons_Lair.mp3',             label: "Dragon's Lair" },
};

/** Pick from the verified packaged pool for a track. */
export function pickTrackFile(track: MusicTrack): string {
  if (!track.variants?.length) return track.file;
  const pool = [track.file, ...track.variants];
  return pool[Math.floor(Math.random() * pool.length)];
}

export const SCREEN_MUSIC: Record<string, string> = {
  title: 'main-theme',
  setup: 'main-theme',
  victory: 'winner',
};

export const LOCATION_MUSIC: Partial<Record<LocationId, string>> = {
  'guild-hall':    'guild-hall',
  'bank':          'bank',
  'slums':         'the-slums',
  'enchanter':     'enchanter',
  'rusty-tankard': 'rusty-tankard',
  'noble-heights': 'noble-heights',
  'cave':          'cave',
};

export const DEFAULT_GAME_TRACK = 'on-the-street';
export const CROSSFADE_MS = 1500;
export const DEFAULT_MUSIC_VOLUME = 0.1;
