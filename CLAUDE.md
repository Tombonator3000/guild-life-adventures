...
- AI education pipeline: `generateProactiveEducationActions` in strategicActions.ts fires REGARDLESS of weakest goal. `generateEducationPipelineActions` allows STARTING new degrees (not just finishing). `getWeakestGoal` redirects career/wealth to education when education progress < 50%. Commitment plan duration for degrees: medium=4, hard=6 turns. Cash flow gate: `costPerSession + 15` (not `* 3`). Work base priority: 55 (not 50), wage boost up to 25 for hard AI.
