...
- Player portrait classes defined in `src/data/portraits.ts` → `PLAYER_PORTRAITS` array. Add new classes there; place matching images in `public/portraits/<id>.jpg`.
- `PLAYER_COLORS` in `game.types.ts` has 6 entries (Crimson, Azure, Emerald, Amber, Violet, Rose). `MAX_TOTAL_PLAYERS` in `GameSetup.tsx` is 6. `gameStore.ts:startNewGame` indexes `PLAYER_COLORS[index]` for human players — never exceed 6 human players without adding more colors first.
