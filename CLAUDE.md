...
- NL chain LOQ resolution: When calling `getQuestLocationObjectives` or `allLocationObjectivesDone` for nlchain quests, pass `player.nlChainProgress` (NOT `player.questChainProgress`). `questChainProgress` is for linear chains only. Using the wrong progress map causes step 0 LOQs to show instead of the current step's LOQs.
