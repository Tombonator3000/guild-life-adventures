...
- Home item art uses the checked-in assets. The unmounted HomeItemGenerator and its orphaned IndexedDB cache were removed in the 2026-09-08 code cleanup.
...
- Tutorial system: `TutorialOverlay` now implements the guided first turn and optional rule-reference cards. It advances after real actions and uses `deriveGameBoardAudienceState` to suppress guidance for spectators. `ContextualTips` remains the contextual hint layer gated by `showTutorial`. Both mount from `GameBoardAuxiliaryLayer`; do not restore the obsolete static-only flow.
...
- Home room layout and art: `src/components/game/home/RoomScene.tsx`.
...
- Short shifts use `WorkplaceCard.tsx` and the canonical `performWorkShift` service. `AcademyPanel.tsx` uses `attendDegreeSession` for standard/cram sessions. Do not restore the unused legacy `WorkSection.tsx` or calculate authoritative prices/effects in the UI.
...
- Cloud shadow wind sync (2026-09-19): `GroundCloudShadows` advances through `TownAtmosphere.drift`, the same integrated wind displacement used by chimney and Enchanter smoke. Keep cloud advection tied to that shared drift rather than independent screen-space directions; the second procedural field may only add subtle silhouette evolution.
- Location background images (2026-03-13): Photo-realistic backgrounds in `src/assets/locations/`. Mapped via `LOCATION_BACKGROUNDS` in `src/assets/locations/index.ts`. Displayed in `LocationShell.tsx` at opacity 0.09 with `mix-blend-mode: multiply`. Old sketch-style images archived as `bg-*-old.jpg` in the same folder. Slums and Noble Heights use original sketch style (not photo). When adding new location backgrounds, use photo-realistic style for consistency.
- Dynamic bank interest (2026-03-17): Savings, investment, and loan interest rates scale with `priceModifier`. Constants renamed: `SAVINGS_WEEKLY_BASE_RATE`, `INVESTMENT_WEEKLY_BASE_RATE`, `LOAN_INTEREST_BASE_RATE`. `processFinances` and `processLoans` in `weekEndHelpers.ts` accept `priceModifier` param. `WeekEndContext` includes `priceModifier`. BankPanel shows current rates. Boom = higher savings return + higher loan cost. Recession = lower both.
- AI trash talk (2026-03-17): `src/data/aiTrashTalk.ts` defines personality-specific one-liners per action trigger. `actionExecutor.ts` fires them via `useBanterStore` after successful AI actions (18% chance, 15s cooldown per AI). Map in `ACTION_TO_TRASH_TALK`. Each personality has ~8-12 lines across different triggers.
- AI dynamic personality scaling (2026-03-17): `getDynamicPersonality()` in `actionGenerator.ts` adjusts gambling/rivalry/caution weights based on wealth ratio vs rivals. Behind (<0.7x avg): +30% gambling, +20% rivalry. Ahead (>1.5x avg): -30% gambling, +20% caution. Applied per-turn, doesn't mutate personality definitions.
- AI strategic location blocking (2026-03-17): `generateStrategicLocationBlocking` in `rivalryActions.ts` — Hard AI (planningDepth >= 3) races to rival's key locations (academy for education-focused rivals, bank/guild-hall for wealth/career). Priority 58-75.
- Player sabotage system (2026-03-17): `SabotagePanel.tsx` at Shadow Market. 3 options: Pickpocket (steal 30g), Distraction (-6h time), Mudslinger (-25 clothing). Costs scale with priceModifier. `sabotagePlayer` store action in `playerHelpers.ts`. Effects applied immediately to target.
- Changelog screen (2026-03-17): `ChangelogScreen.tsx` — versioned changelog (v0.1.0–v0.9.0) accessible via "What's New" button on TitleScreen (below tertiary row). Lazy-loaded. Collapsible version blocks with icon-coded entries. Update `CHANGELOG` array in ChangelogScreen.tsx when adding new features.
- Reputation system (2026-03-19): Two-axis Fame (0-100) & Infamy (0-100) in `src/data/reputation.ts`. Fame from quests/dungeon/education/work. Infamy from sabotage/hexes/dark rituals. Effects: price modifiers at shops (`getReputationPriceMultiplier`), NPC greeting overrides (`getReputationGreeting` in LocationShell), exclusive services at locations (ReputationPanel, auto-injected "Renown" tab). Save version 7 (v6→v7 adds fame/infamy/purchasedReputationUnlocks). Access with `(player.fame ?? 0)` in UI for old-save safety. AI earns reputation passively + has `buy-reputation-unlock` action type.
- SabotagePanel redesign (2026-03-19): Changed from repeated per-rival sections to single shadcn Select dropdown for target selection. Cleaner UI with 4+ opponents.
- Shadowfingers NPC overhaul (2026-03-19): Shadowfingers is the thematic NPC for all robbery/sabotage. `ShadowfingersToken.tsx` animates from board edge to victim during robbery events. Sabotage options branded as "Hire Shadowfingers". Robbery expanded to 6 risk-zone locations (bank highest, forge lowest). Safe zones: noble-heights, academy, guild-hall, etc. `ROBBERY_CHANCES` map in `shadowfingers.ts`.
- Fence protection system (2026-03-19): `FenceProtectionPanel.tsx` — buy protection money (3/6/10 weeks, -80% robbery chance) or tip-offs (rival vulnerability intel via `getRobberyVulnerability`). `player.protectionWeeksLeft` field, decays in `resetWeeklyFlags`. Save version 8 (v7→v8). Sabotage also available at Fence (not just Shadow Market).
- `protectionWeeksLeft` on Player: access with `(player.protectionWeeksLeft ?? 0)` in UI for old-save safety. Store action: `buyProtection(playerId, weeks, cost)` in playerHelpers.
- Store action gold validation (2026-04-13): ALL store actions that deduct gold (`sabotagePlayer`, `buyProtection`, `purchaseReputationUnlock`) MUST validate `player.gold >= cost` before mutating state. The UI disables buttons when gold is insufficient, but the store guard prevents multiplayer exploits where a guest sends the action directly.
- `purchaseReputationUnlock` effect capping (2026-04-13): happiness from reputation unlocks must be capped at `Math.min(100, ...)`. All effect types (happiness, health, clothing, food) must be capped at their respective maximums.
- `ReputationUnlock.effect` is single-type only: each unlock has ONE effect (`happiness | health | gold | clothing | food`). Do NOT promise multiple effects in descriptions (e.g., "+25 happiness, +50 food") unless the code applies both. Use `purchaseReputationUnlock` which only applies the single `effect.type`.
- AI Fence services (2026-04-19): AI can use `buy-protection`, `buy-tip-off`, and `sabotage-player` actions. Handlers in `src/hooks/ai/handlers/fenceHandlers.ts`. Generators in `rivalryActions.ts` (`generateProtectionPurchase`, `generateSabotageActions`, `generateTipOffPurchase`). Protection: any AI when gold>=200 and no active protection (caution-weighted). Sabotage/Tip-off: Hard AI only with `rivalry >= 1.0` (Thornwick/Morgath/Grimwald). `actionKey()` in `useGrimwaldAI.ts` includes `targetId` to avoid retry-loop on same sabotage target.
- Save migration mandatory rule (2026-04-28): EVERY new field added to `Player` (or `GameState`) MUST get a save-migration block in `src/data/saveLoad.ts` AND bump `SAVE_VERSION`. Without this, old saves load with `undefined` for the new field and crash on first read (`player.foo[bar]` → TypeError). The v8→v9 fix backfilled `nlChainProgress`, `nlChainCompleted`, `pendingNLChainChoice`, and `backupOutfit` which were added without migrations between v4 and v8. Pattern: `if (saveData.version < N) { for (const p of saveData.gameState.players) { if (p.newField === undefined) p.newField = <default>; } saveData.version = N; }`. UI code that reads new fields should also use defensive optional chaining (`player.foo?.[bar] ?? default`) as a second line of defense.
