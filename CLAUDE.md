...
- Weekend event messages are filtered to max 5 via `limitWeekendMessages()` in `weekEndHelpers.ts`. Critical events (eviction, starvation, death, robbery, loan default, CRASH) always fill slots before ordinary ones (weekend activity, lottery, weather). Weekend events are shown on ONE screen in `GameBoard.tsx` (no pagination) — `isWeekendEvent = eventSource === 'weekend'` skips the queue and passes full description to `EventPanel` at once.
